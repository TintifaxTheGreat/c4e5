...
func TestTestCapture(t *testing.T) {
	g := NewGame("4kN2/8/7K/b5B1/2N2R2/6rn/2K5/8 w - - 0 1", 0, 0, 0)
	...
	g = NewGame("4kN2/4P3/7K/b5B1/2N2R2/6rn/2K5/8 b - - 0 1", 0, 0, 0)
	moves = g.Board.GenerateLegalMoves()
	captures = nil
	for _, move := range moves {
		if testCapture(move, &g.Board) {
	...
	g = NewGame("4kN2/5P2/7K/b5B1/2N2R2/6rn/2K5/8 b - - 0 1", 0, 0, 0)
	moves = g.Board.GenerateLegalMoves()
	captures = nil
	for _, move := range moves {
		if testCapture(move, &g.Board) {

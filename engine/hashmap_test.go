...
func TestGet(t *testing.T) {
	...
	for _, move := range moves {
		...
		values[move] = -negamax(&g.Board, h, depth, -beta, -alpha)
		h.Put(depth, values[move], &g.Board)
		unapplyFunc()
	}
	...
	for _, move := range sortedMoves {
		unapplyFunc := g.Board.Apply(move)
		storedValue, _ := h.Get(depth, &g.Board)
		storedValue1, _ := h.Get(depth-1, &g.Board)
		_, ok2 := h.Get(depth+1, &g.Board)

		calculatedValue := -negamax(&g.Board, h, depth, -beta, -alpha)
